...
st.set_page_config(page_title="PET Resin Komple ERP", layout="wide")
...
def toplam_hammadde_stok():
    ...
    if df.empty: return {"PTA": 0.0, "MEG": 0.0}
    return df.groupby("Hammadde")["Miktar (Ton)"].sum().to_dict()
...
st.sidebar.title("🧪 PET Resin ERP v2.0")
st.sidebar.write("---")
...
if sayfa == "📊 Genel Depo & Stok Durumu":
    ...
    st.subheader("💡 Kritik Stok Özetleri")
    c1, c2, c3 = st.columns(3)
    c1.metric("Mevcut PTA Stoku", f"{h_stok.get('PTA', 0.0):.2f} Ton")
    c2.metric("Mevcut MEG Stoku", f"{h_stok.get('MEG', 0.0):.2f} Ton")
    
    mamul_toplam = sum(m_stok.values()) if m_stok else 0.0
    c3.metric("Toplam Satışa Hazır Mamul Stoku", f"{mamul_toplam:.2f} Ton")
...
# ==========================================
    ...
    with st.form("hammadde_form"):
        ...
        h_turu = st.selectbox("Hammadde Türü", ["PTA", "MEG", "Katalizör", "Katkı Maddesi"])
        h_lot = st.text_input("Hammadde LOT Numarası (Örn: LOT-PTA-2026)")
        h_miktar = st.number_input("Gelen Miktar (Ton)", min_value=0.1, step=1.0)

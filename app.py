...
# Üretim esnasında harcanan lot detaylarını varyans için tutacak yeni kayıt günlüğü
...
# --- TEK PARÇA ÇOKLU SHEET DESTEKLİ EXCEL RAPOR MOTORU ---
def endustriyel_excel_rapor_olustur(bas_tarih, bit_tarih):
    ...
    def tarih_filtrele(df, tarih_kolonu):
        ...
        df_copy = df.copy()
        # Tarih formatlarını güvenli eşitleme
        df_copy[tarih_kolonu] = df_copy[tarih_kolonu].apply(lambda x: x.split(" ")[0] if " " in str(x) else str(x))
    ...
    # Mevcut Anlık Özet Dengesi (Anlık Durum)
    stok_durumu = segment_stok_getir()
    bakiye_satirlari = []
    for malz, kalan_stok in stok_durumu.items():
        bakiye_satirlari.append({
            "Malzeme / Ürün Adı": malz,
            "Mevcut Depo Stoğu": kalan_stok,
            "Ölçü Birimi": malzeme_birimi_bul(malz),
            "Toplam Harcanan Ölçü": st.session_state.hammadde_kullanilan_toplam.get(malz, 0.0)
        })
    df_anlik_bakiye = pd.DataFrame(bakiye_satirlari)

    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        if not df_anlik_bakiye.empty:
            df_anlik_bakiye.to_excel(writer, index=False, sheet_name='Anlık Depo Bakiyeleri Özet')
        if not f_depo_giris.empty:
            f_depo_giris.to_excel(writer, index=False, sheet_name='Dönemsel Giriş Hareketleri')
        if not f_uretim_harcama.empty:
            f_uretim_harcama.to_excel(writer, index=False, sheet_name='Dönemsel Tüketim Detayları')
        if not f_mamul_depo.empty:
            f_mamul_depo.to_excel(writer, index=False, sheet_name='Dönemsel Mamul Üretimleri')
...
# ==========================================
# YENİ SAYFA: FABRİKA RAPORLAR SAYFASI (TARİH ARALIKLI EXCEL ÇIKTISI)
# ==========================================
elif sayfa == "📈 📊 Fabrika Raporlar Sayfası":
    st.header("📈 📊 Dönemsel Fabrika Üretim ve Stok Hareketleri Rapor İstasyonu")
    st.write("Belirttiğiniz tarih aralığına göre hammadde azalmalarını, üretim sarfiyatlarını ve mamul artışlarını tek bir Excel dosyasında inceleyin.")
    ...
    if bas_secim > bit_secim:
        ...
        st.write("---")
        st.subheader("📥 Raporu Dışarı Aktar")
        
        # Excel oluşturucu fonksiyonu çağırıyoruz
        excel_dosyası = endustriyel_excel_rapor_olustur(bas_secim, bit_secim)
        
        st.download_button(
            label="📊 Fabrika Konsolide Genel Stok Raporunu İndir (.XLSX)",
            data=excel_dosyası,
            file_name=f"Fabrika_Genel_Sistem_Raporu_{bas_secim}_to_{bit_secim}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
        st.success("ℹ️ İndireceğiniz tek bir Excel dosyasının içerisinde alt sekmeler halinde 'Anlık Depo Bakiyeleri Özet', 'Giriş Hareketleri', 'Tüketim Detayları' ve 'Mamul Üretimleri' yer alacaktır.")
...
# ==========================================
    ...
    if operasyon_turu == "✏️ Mevcut Reçeteyi Gör ve Düzenle" and st.session_state.receteler:
        ...
        r_adi, r_turu, eski_bom = st.session_state.receteler[duzenleme_indeksi]["Reçete Adı"], st.session_state.receteler[duzenleme_indeksi]["Tür"], st.session_state.receteler[duzenleme_indeksi]["BOM"]
...
# ==========================================
# SAYFA 3: ÜRETİM EMRİ & GİRİŞİ (RAPORLAMA GÜNLÜĞÜ ENTEGRE EDİLDİ)
# ==========================================
        ...
        with st.form("uretim_form"):
            ...
            if st.form_submit_button("Üretimi Onayla"):
                ...
                if not kontrol: st.error("❌ Stok Yetersiz!")
                    ...
                    # Tüketimi düşme ve TARİH BAZLI RAPOR İÇİN GÜNLÜĞE YAZMA
                    for h_adi, f_amt in fiili_girisler.items():
                        ...
                        st.session_state.uretim_harcamalari_log.append({
                            "Tarih": current_date_str, "Üretim LOT": u_lot, "Harcanan Malzeme": h_adi, "Miktar": f_amt, "Birim": malzeme_birimi_bul(h_adi)
                        })
